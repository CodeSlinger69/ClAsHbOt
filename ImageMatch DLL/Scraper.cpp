...
{
	char s[MAX_PATH];
	sprintf_s(s, MAX_PATH, "%s\\ImageFiles.data", scriptDir);
	imageDataPath = s;
    ...
    if (!f.is_open())
	{
		...
		char s[500];
		sprintf_s(s, 500, "Error opening %s: %s", imageDataPath.c_str(), err);
		logger->WriteLog(s);
		return;
	}
    ...
    while (getline(f, line))
	{
		...
		{
			char s[500];
			sprintf_s(s, 500, "Image group %s is unrecognized.", elems[0].c_str());
			logger->WriteLog(s);
		}
		...
		else
		{
			...
			for (int i = 1; i < (int) elems.size(); i++)
			{
				...
				char path[MAX_PATH];
				sprintf_s(path, MAX_PATH, "%s\\Images\\%s", scriptPath, elems[i].c_str());
				imageGroups[lastGroup].mats[i-1] = imread(path);
			}
    ...
    if (f.bad())
	{
		...
		strerror_s(err, 500, errno);
		char s[500];
		sprintf_s(s, 500, "Error reading %s: %s", imageDataPath.c_str(), err);
		logger->WriteLog(s);
	}
